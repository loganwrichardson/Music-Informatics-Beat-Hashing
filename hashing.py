...
def get_timbre_edges(segments):
    ...
    if timbre_edges is None:
        timbre_edges = np.percentile(
            a=segments['timbre'],
            q=np.linspace(0, 100, timbre_bins + 1),
            axis=0)[1:-1].T
    return timbre_edges
...
def hash_function_1(beats, segments):
    ...
    all_of_these = [
        ...
        hf_num_segments_gt_0(beats, segments)
...
def get_timbre(beats, segments, segment_index, coefficients):
    """
    Return the designated "timbre_coefficients" for the "segment_index"-th segment in every beat.
...
def timing(num_beats):
    ...
    return np.interp(num_beats, n, t, left=3.076085864793597e-05, right=t[-1]+slope*(num_beats-n[-1])) * num_beats
